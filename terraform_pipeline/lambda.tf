...
resource "aws_lambda_function" "jet_staging_lambda" {
  ...
  memory_size   = 1024
  timeout       = 120
  image_uri     = "605126261673.dkr.ecr.eu-west-2.amazonaws.com/jet-staging-repo:latest"
  package_type  = "Image"
...
# Create staging schedule target for lambda function 
...
resource "aws_lambda_function" "jet_production_lambda" {
  ...
  memory_size   = 1024
  timeout       = 120
  image_uri     = "605126261673.dkr.ecr.eu-west-2.amazonaws.com/jet-production-repo:latest"
  package_type  = "Image"
  architectures = ["x86_64"]
